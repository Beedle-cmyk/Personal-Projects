...
class Trainer:
    ...
    def validate_cuda():
        """
        Helper for checking if the client is running the model on their cpu/gpu and prompting
    ...
    def train(self, cfg : Path | str="args.yaml", current_project_dir : Path | str | None=None):
        """
        Base method for training a model using the provided parameters from an yaml file

        Args:
            cfg : configuration file with arguments/parameters to train model with
            current_project_dir: path to current working project

        Returns:
            Results object generated by model
        """
        return self._run(cfg=cfg, current_project_dir=current_project_dir, tune=False)



    def tune(self, cfg="args.yaml", current_project_dir=None):
        """
        ...
        return self._run(cfg=cfg, current_project_dir=current_project_dir, tune=True)

    

    def _run(self, cfg : Path | str="args.yaml", current_project_dir : Path | str | None=None, tune : bool=False):
        """
        Internal Method for running either the YOLO tune or train calls

        Args:
            cfg (Path | str): yaml configuration file path 
        ...
        project_dir = Path(current_project_dir or Path.cwd())
