...
class Trainer:
    ...
    def train(self, cfg : Path | str="args.yaml", current_proj_dir : Path | str | None=None):
        """
        ...
        return self._run(cfg=cfg, current_proj_dir=current_proj_dir, tune=False)
    ...
    def _run(self, cfg : Path | str="args.yaml", current_proj_dir : Path | str | None=None, tune : bool=False):
        """
        ...
        checkpoints = sorted(run_dir.rglob("last.pt"), reverse=True)  # so that latest checkpoint is first
        checkpoint = checkpoints[0] if checkpoints else None

        for attempt in range(MAX_RETRIES):
            try:
                resume = tune and run_dir.exists()
                print(f"{'Resuming' if resume else 'Starting'} {name} run")
